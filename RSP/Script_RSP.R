...
# for Mike
setwd("C:/Git/ETN_workshop_Jan2021/RSP/")
...
# we're using WGS84, so size is in latitude degrees
# buffer argument adds a buffer around your shapefile, in the case there is "error" in location
# information and some observations fall outside your shapefile
...
rsp.data$detections$`A69-9004-489`[1:11, ] # note the interpolated RSP "detections"

# Plot RSP track
rsp.data$track$`A69-9002-10474` 
plotTracks(input = rsp.data, 
           base.raster = water, 
           tag = "A69-9002-10474")
plotTracks(input = rsp.data, 
           base.raster = water, 
...
plotTracks(input = rsp.data, 
           ...
           tag = "A69-9004-485", 
           track = 3) + 
	addStations(rsp.data) + 
  addRecaptures(Signal = "485") # Also adds the recapture locations
plotTracks(input = rsp.data, 
           base.raster = water, 
           tag = "A69-9004-485", 
           track = 6) + 
  addStations(rsp.data) + 
  addRecaptures(Signal = "485") 
...
#==============================================================================#
dbbmm.total <- dynBBMM(input = rsp.data, 
                       base.raster = water, 
                       UTM = 56, 
                       verbose = TRUE,
                       start.time = "2014-11-01 00:00:00", 
                       stop.time = "2014-11-05 00:00:00") # Choose part of the tracking period

names(dbbmm.total)
dbbmm.total$valid.tracks

# Plot dBBMM models
plotContours(input = dbbmm.total, 
             tag = "A69-9004-489") # Group A dBBMM

plotContours(input = dbbmm.total, 
             tag = "A69-9002-10480",
             land.col = "#55903b60", 
             col = cmocean('solar')(4)) # Group B dBBMM: custom colours

plotContours(input = dbbmm.total, 
             tag = "A69-9004-485", 
             land.col = "#55903b60",
             title = "Animal 485", 
             scale.type = "continuous") # Group B dBBMM: levels with a continuous scale

# RSP plots are ggplot2! :) 
plotContours(input = dbbmm.total, 
             tag = "A69-9002-10480", 
             land.col = "#55903b60", 
             title = "Group B: animal 10480") +
  ggsn::scalebar(x.min = 151.5, 
                 x.max = 151.55, 
                 y.min = -32.95, 
                 y.max = -32.92, 
                 transform = TRUE,
...
plotContours(input = dbbmm.total, 
             tag = "A69-9002-10480", 
             land.col = "#55903b60", 
             title = "2014-11-01 | 2014-11-05",
             breaks = c(0.99, 0.95, 0.90, 0.70, 0.50, 0.25, 0.20), 
             col = rev(cmocean('thermal')(7))) +
	ggsn::scalebar(x.min = 151.5, 
	               x.max = 151.55, 
	               y.min = -32.95, 
	               y.max = -32.92, 
...
aux.raster.latlon <- projectRaster(aux.raster, 
                                   crs = CRS("+proj=longlat +datum=WGS84 +no_defs ")) # Reproject to latlong!
plot(aux.raster.latlon)

raster.df <- as.data.frame(aux.raster.latlon, 
                           xy = TRUE) # Convert raster to a dataframe
...
ggplot() + 
  theme_minimal() +
	geom_raster(data = raster.df, 
	            aes(x = x, y = y, fill = dBBMM)) +
		scale_fill_gradientn(colors = rev(cmocean('thermal')(100))) +
		geom_polygon(data = Lake, 
		             aes(x = long, y = lat), 
		             color = NA, 
		             fill = "#38bd8d40", 
		             alpha = 0.5, 
		             size = 0.2) +
		coord_cartesian(xlim = c(151.47, 151.7), 
		                ylim = c(-33.18, -32.9), 
		                expand = FALSE) +
		labs(x = "Longitude", 
		     y = "Latitude", 
		     title = "A69-9002-10480 (2014-11-01 | 2014-11-05)") +
		ggsn::scalebar(x.min = 151.52, 
		               x.max = 151.57, 
		               y.min = -32.95, 
		               y.max = -32.92, 
		               transform = TRUE, 
...
dbbmm.total$valid.tracks 
areas.track <- getAreas(dbbmm.total, 
                        type = "track")
areas.track$areas

areas.tot <- getAreas(dbbmm.total, 
                      type = "group", 
                      breaks = c(0.25, 0.5, 0.95))
areas.tot$areas
...
plot1 <- plotContours(input = dbbmm.daily, 
                      tag = "A69-9004-485", 
                      timeslot = 1,
                      title = "Tag 485 (2014-11-01)", 
...
plot2 <- plotContours(input = dbbmm.daily, 
                      tag = "A69-9004-485", 
                      timeslot = 2,
                      title = "Tag 485 (2014-11-02)", 
                      scale.type = "continuous") +
  ggsn::scalebar(x.min = 151.5, 
                 x.max = 151.55, 
                 y.min = -32.95, 
...
plot3 <- plotContours(input = dbbmm.daily, 
                      tag = "A69-9004-485", 
                      timeslot = 3,
                      title = "Tag 485 (2014-11-03)", 
                      scale.type = "continuous") +
  ggsn::scalebar(x.min = 151.5, 
                 x.max = 151.55, 
                 y.min = -32.95, 
...
plot4 <- plotContours(input = dbbmm.daily, 
                      tag = "A69-9004-485", 
                      timeslot = 4,
                      title = "Tag 485 (2014-11-04)", 
                      scale.type = "continuous") +
	ggsn::scalebar(x.min = 151.5, 
	               x.max = 151.55, 
	               y.min = -32.95, 
...
areas.daily <- getAreas(dbbmm.daily, 
                        type = "group")
areas.daily$areas

plot1 <- plotAreas(areas.daily, 
                   base.raster = water, 
                   group = "A", 
                   timeslot = 1, 
                   land.col = "#55903b60",
...
df.centroid.B <- getCentroids(input = dbbmm.daily, 
                              areas = areas.daily, 
                              type = "group", 
                              level = 0.5, 
                              group = "B", 
...
plotAreas(areas.daily, 
          base.raster = water, 
          group = "B", 
          timeslot = 2, 
          title = "Group B (2014-11-02)") +
		addCentroids(input = df.centroid.B, 
		             timeslot = 2)

# Obtain dBBMM centroids also at track level: but addCentroids only works for group! 
areas.track <- getAreas(dbbmm.daily, 
                        type = "track")
areas.track$areas

df.centroid.track <- getCentroids(input = dbbmm.daily, 
                                  areas = areas.track, 
                                  type = "track", 
                                  level = 0.5, 
                                  UTM = 56)
...
plotOverlaps(over.daily, 
             ...
             level = 0.5, 
             timeslot = 4, 
             title = "Overlap at 50% dBBMM on 2014-11-04")


# Extract overlap data for further investigation:
df.aux.5 <- getOverlapData(input = over.daily, 
                           dbbmm = dbbmm.daily, 
                           groups = c("A", "B"), 
                           level = 0.5)
df.aux.5
df.aux.95 <- getOverlapData(input = over.daily, 
                            dbbmm = dbbmm.daily, 
                            groups = c("A", "B"), 
                            level = 0.95)
df.aux.95
...
ggplot() + 
  theme_bw() +
	geom_line(data = df.aux, 
	          aes(x = start, y = Absolute_A_B, colour = Level)) +
...
ggplot() + 
  theme_bw() +
	geom_line(data = df.aux, 
	          aes(x = start, y = Percentage_A_B, colour = Level)) +
	labs(y = "Percentage overlap between A and B (%)", 
	     x = "Date")
